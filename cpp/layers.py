...
def affine_forward(x, w, b):
    ...
    for i in range(len(x_shape)):
      reshape_size = reshape_size * x_shape[i]
    X = x.reshape((N,reshape_size))
    out = np.dot(X, w) + b
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = {'x':x, 'z':x_hat, 'gamma':gamma, 'std':std, 
                 'var':var, 'axis':layernorm, 'mean':mu}
...
def batchnorm_backward(dout, cache):
    ...
    dstd_2 = np.sum((dx_hat * (cache['x'] - cache['mean']) * (-0.5)*(cache['var'])**(-1.5)), axis=0)
    dmu = np.sum(dx_hat,axis=0) * -1/np.sqrt(cache['var']) + dstd_2*1/N *np.sum((2*(-cache['x']+cache['mean'])),axis=0)
    dx = dx_hat * 1/np.sqrt(cache['var']) + dstd_2*2/N*(cache['x']-cache['mean']) + dmu/N
